...
def complex_recipe_search(query, number):
    """ Returns a python dict containing Spoonacular Compley Search Results for given arguments"""
    # resp = requests.get(f"https://api.spoonacular.com/recipes/complexSearch?apiKey=3de123cdc6f14ce0a9bc46a5e5edeb2d&query={query}&number={number}") 
...
def recipe_detail_search(recipe_spoonacular_id):
    ...
    ingredients = [{'spoonacular_ingredient_id': ingredient['id'], 'name': ingredient['name'], 'image': ingredient['image'], 'amount' : f"{ingredient['measures']['metric']['amount']} {ingredient['measures']['metric']['unitLong']}"} for ingredient in response_dict['extendedIngredients']]
